...
class Customers extends Model
{
    ...
    public function get_custtomerInfo($openid){
        $list="
                SELECT b.detail,a.id AS id,a.company,a.charger,a.remark,a. SCHEDULE,a.scale,a.position,date_format(a.create_time, '%Y-%m-%d') AS create_time,date_format(a.update_time, '%Y-%m-%d') AS update_time
                FROM
    ...
    public function get_customer_Records($openid){
        $list="
                SELECT a.detail,c.id AS id,c.company,c.charger,c.remark,c. SCHEDULE,c.scale,c.position,date_format(c.create_time, '%Y-%m-%d') AS create_time,date_format(c.update_time, '%Y-%m-%d') AS update_time
                FROM
    ...
    public function CustomerList1($openid){
        $info=$this->get_custtomerInfo($openid);
        $records=$this->get_customer_Records($openid);
        $res=Db::query("
                        SELECT DISTINCT(t2.id),t2.detail,t2.company,t2.charger,t2.remark,t2. SCHEDULE,t2.scale,t2.position,t2.create_time,t2.update_time
                        FROM
                            (
                                $info UNION ALL $records
                            ) AS t2
                        ORDER BY
                            t2.create_time DESC
                     ");
